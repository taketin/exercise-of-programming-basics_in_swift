...
class Chapter12 {
    // 問題 12.4
    func seiretsu(lst: [Ekimei], resultLst: [Ekimei] = []) -> [Ekimei] {
        ...
        return seiretsu(lst: Array(copiedLst), resultLst: insert(lst: resultLst, addEkimei: ekimei))
    }

    func insert(lst: [Ekimei], addEkimei: Ekimei, cursor: Int = 0) -> [Ekimei] {
        if lst.count == 0 {
            return [addEkimei]
        }

        if lst[cursor].kana == addEkimei.kana {
            return lst
        }

        if lst[cursor].kana > addEkimei.kana {
            var copiedLst = lst
            copiedLst.insert(addEkimei, at: cursor)
            return copiedLst
        }
        ...
        if lst[cursor].kana < addEkimei.kana {
            ...
            guard lst.count > cursor + 1 else {
                copiedLst.append(addEkimei)
                return copiedLst
            }

            if lst[cursor + 1].kana > addEkimei.kana {
                copiedLst.insert(addEkimei, at: cursor + 1)
                return copiedLst
            }
        }

        return insert(lst: lst, addEkimei: addEkimei, cursor: cursor + 1)
    }

    // 問題 12.3
    func shokika(lst: [Eki], kiten: String, resultLst: [Eki] = [], shokikaSeikou: Bool = false) -> [Eki]? {
        guard lst.count > 0 else {
        ...
        if kiten == eki.namae {
            targetEki = Eki(namae: eki.namae, saitanKyori: 0, temaeList: [eki.namae])
            shokikaSeikouFlag = true
        } else {
            targetEki = eki
        }
...
class Chapter12Test: XCTestCase {
    func testSeiretsu() {
        let chapter12 = Chapter12()

        let ekimei1 = Ekimei(kanji: "池袋", kana: "いけぶくろ", romaji: "ikebukuro", shozoku: "丸ノ内線")
        let ekimei2 = Ekimei(kanji: "東池袋", kana: "ひがしいけぶくろ", romaji: "higasiikebukuro", shozoku: "有楽町線")
        let ekimei3 = Ekimei(kanji: "池袋", kana: "いけぶくろ", romaji: "ikebukuro", shozoku: "有楽町線")
        let ekimei4 = Ekimei(kanji: "二重橋前", kana: "にじゅうばしまえ", romaji: "nijuubasimae", shozoku: "千代田線")

        XCTAssertEqual(chapter12.seiretsu(lst: [ekimei1,ekimei2,ekimei3,ekimei4]), [ekimei1,ekimei4,ekimei2])
    }
    ...
    func testInsert() {
        ...
        XCTAssertEqual(chapter12.insert(lst: [ekimei1,ekimei3,ekimei2], addEkimei: ekimei4), [ekimei1,ekimei3,ekimei4,ekimei2])
    }

    func testShokika() {
        let chapter12 = Chapter12()

        let eki1 = Eki(namae: "代々木上原", saitanKyori: MAXFLOAT, temaeList: [])
        let eki2 = Eki(namae: "代々木公園", saitanKyori: MAXFLOAT, temaeList: [])
        let eki3 = Eki(namae: "明治神宮前", saitanKyori: MAXFLOAT, temaeList: [])

        var lst = [eki1, eki2, eki3]
        var expectedLst = [
            Eki(namae: "代々木上原", saitanKyori: 0, temaeList: ["代々木上原"]),
            eki2,
            eki3
        ]
        XCTAssertEqual(chapter12.shokika(lst: lst, kiten: "代々木上原"), expectedLst)
        XCTAssertEqual(chapter12.shokika(lst: lst, kiten: "ほげほげ"), nil)

        lst = []
        expectedLst = []
        XCTAssertEqual(chapter12.shokika(lst: [], kiten: "代々木上原"), nil)
    }

    func testMakeEkiList() {
        let chapter12 = Chapter12()

        let ekimei1 = Ekimei(kanji: "代々木上原", kana: "よよぎうえはら", romaji: "yoyogiuehara", shozoku: "千代田線")
        let ekimei2 = Ekimei(kanji: "代々木公園", kana: "よよぎこうえん", romaji: "yoyogikouen", shozoku: "千代田線")
        let ekimei3 = Ekimei(kanji: "明治神宮前", kana: "めいじじんぐうまえ", romaji: "meijijinguumae", shozoku: "千代田線")

        var lst = [ekimei1, ekimei2, ekimei3]
        var expectedLst = [
            Eki(namae: "代々木上原", saitanKyori: MAXFLOAT, temaeList: []),
            Eki(namae: "代々木公園", saitanKyori: MAXFLOAT, temaeList: []),
            Eki(namae: "明治神宮前", saitanKyori: MAXFLOAT, temaeList: [])
        ]
        XCTAssertEqual(chapter12.makeEKiList(lst: lst), expectedLst)
...
Chapter12Test.defaultTestSuite.run()
